...
Services khởi động: Kafka (:9092), Kafka UI (:8080), PostgreSQL (:5433), Redis (:6379)

### 2. Khởi tạo Kafka Topics

```bash
go run cmd/kafka-topic-init/main.go
```

Tạo 4 topics: `order.created`, `order.created.DLQ`, `order.payment.processed`, `order.payment.processed.DLQ`

### 3. Khởi tạo Database Schema

Migration chạy tự động qua `docker-entrypoint-initdb.d` khi container PostgreSQL được tạo lần đầu.

Nếu đã có container cũ, chạy:
```bash
docker exec -i postgres psql -U postgres -d order_processing < migrations/001_init.sql
```

### 4. Chạy các service (mỗi service 1 terminal)

```bash
# Terminal 1: Order Service REST API
go run cmd/order-service/main.go

# Terminal 2: Outbox Relay (poll & publish)
go run cmd/outbox-relay/main.go

# Terminal 3: Payment Service (consumer)
go run cmd/payment-service/main.go

# Terminal 4: Notification Service (consumer)
...
curl -X POST http://localhost:3000/orders \
  ...
  -d '{"user_id":"user-001","product_id":"product-001","amount":99.99}'
```
...
```bash
go run cmd/kafka-bad-message-test/main.go
```
